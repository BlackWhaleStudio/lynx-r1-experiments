...
namespace Lynx.UI
{
    public class ButtonAnimationMethods
    {
        ...
        public static IEnumerator PressingAnimationCoroutine(ButtonAnimation buttonAnimation, Transform transform, Action<bool> callback)
        {
            ...
            Vector3 moveBasePos = !buttonAnimation.isUsingScale ? buttonAnimation.moveBasePos : Vector3.Scale(buttonAnimation.moveBasePos, LynxVectors.InverseVector(transform.localScale));
            Vector3 moveDelta = !buttonAnimation.isUsingScale ? buttonAnimation.moveDelta : Vector3.Scale(buttonAnimation.moveDelta, LynxVectors.InverseVector(transform.localScale));
        ...
        /// <summary>
        /// Call this function to set the move root.
        /// </summary>
        /// <param name="buttonAnimation">Button animation values.</param>
        /// <param name="transform">Transform of the button.</param>
        public static void SetMoveRoot(ButtonAnimation buttonAnimation, Transform transform)
        {
            buttonAnimation.moveRoot.localPosition = !buttonAnimation.isUsingScale ? buttonAnimation.moveBasePos : Vector3.Scale(buttonAnimation.moveBasePos, LynxVectors.InverseVector(transform.localScale));
        }

        /// <summary>
        /// Call this function to set the move root with a simple transform.
        /// </summary>
        /// <param name="buttonAnimation"></param>
        /// <param name="transform">Transform of the target graphic.</param>
        public static void MoveRootAutoCompletion(ButtonAnimation buttonAnimation, Transform transform)
        {
            if (buttonAnimation.moveRoot == null)
            {
                buttonAnimation.moveRoot = transform;
            }
        }
    }
}
